...
def primitive_to_shape(p, in_region=False, simplify_regions=False):
    ...
    if type(p) == primitives.Line:
        ...
        raise NotImplementedError(
            "Unexpected primitive type {}".format(type(p)))
    return vertices
...
def process(outline_file, solderpaste_file, stencil_thickness=0.2, include_ledge=True,
            ledge_height=1.2, ledge_gap=0.0, increase_hole_size_by=0.0, simplify_regions=False):
    ...
    if include_ledge:
        ...
        ledge_polygon = offset(1)(ledge_polygon - polygon(cutter))
